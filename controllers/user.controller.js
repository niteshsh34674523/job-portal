...
export const updateProfile = async (req, res) => {
  try {
    ...
    // ✅ Upload resume to Cloudinary (handles PDFs and images)
    if (file) {
      const fileUri = getDataUri(file);
      const isPDF = file.mimetype === "application/pdf";

      const cloudResponse = await cloudinary.uploader.upload(fileUri.content, {
        resource_type: isPDF ? "raw" : "auto", // <-- KEY FIX
        folder: "resumes",
      });

      user.profile.resume = cloudResponse.secure_url; // ✅ Cloud URL (not system path)
